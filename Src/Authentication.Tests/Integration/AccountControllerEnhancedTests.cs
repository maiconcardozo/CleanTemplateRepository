...
namespace Authentication.Tests.Integration
{
    public class AccountControllerEnhancedTests : IClassFixture<AuthenticationWebApplicationFactory>
    {
        ...
        {
            // Arrange
            var request = new StringContent("", Encoding.UTF8);
            request.Headers.ContentType = null;

            // Act
            var response = await _client.PostAsync("/Account/AddAccount", request);
        ...
        public async Task GetAccountById_WithVeryLargeId_ShouldReturnNotFound()
        {
            ...
            // Act
            var response = await _client.GetAsync($"/Account/GetAccountById/{largeId}");
        ...
        public async Task GetAccountById_WithNegativeId_ShouldReturnBadRequest()
        {
            ...
            // Act
            var response = await _client.GetAsync($"/Account/GetAccountById/{negativeId}");
        ...
        {
            // Act
            var response = await _client.GetAsync("/Account/GetAccountById/0");
        ...
        public async Task AddAccount_ConcurrentRequests_ShouldHandleCorrectly()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                tasks.Add(_client.PostAsync("/Account/AddAccount", content));
            }
