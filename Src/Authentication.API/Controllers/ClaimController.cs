...
namespace Authentication.API.Controllers
{
    ...
    public class ClaimController : ControllerBase
    {
        ...
        [SwaggerResponseExample(StatusCodes.Status404NotFound, typeof(ProblemDetailsNotFoundExample))]
        [SwaggerResponseExample(StatusCodes.Status500InternalServerError, typeof(ProblemDetailsInternalServerErrorExample))]
        public IActionResult UpdateClaim([FromBody] ClaimPayLoadDTO claimDTO, [FromServices] IServiceProvider serviceProvider)
        {
            try
            {
                var existingClaim = claimService.GetById(claimDTO.Id);
                ...
                claim.CreatedBy = existingClaim.CreatedBy;
                claim.DtCreated = existingClaim.DtCreated;
        ...
        public IActionResult DeleteClaim(int id)
        {
            try
            {
                var existingClaim = claimService.GetById(id);
                if (existingClaim == null)
                {
                    var notFoundDetails = ProblemDetailsExampleFactory.ForNotFound(ResourceAPI.ClaimNotFound, HttpContext.Request.Path);
                    return NotFound(notFoundDetails);
