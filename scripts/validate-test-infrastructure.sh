...
if [ -f "docs/guides/TESTING.md" ]; then
    print_status "PASS" "Testing documentation exists"
    
    # Check for key sections
    if grep -q "Quick Test Execution" docs/guides/TESTING.md; then
    ...
    if grep -q "Troubleshooting" docs/guides/TESTING.md; then
...
if [ -f "build/global.json" ]; then
    print_status "PASS" "global.json exists"
    
    if grep -q '"version": "9.0' build/global.json; then
...
if [ $CHECKS_FAILED -eq 0 ]; then
    ...
    echo "📚 See docs/guides/TESTING.md for detailed setup instructions."
    exit 1
fi